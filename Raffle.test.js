...
describe("Raffle", () => {
    ...
    beforeEach(async () => {
        ...
        await vrfCoordinatorV2Mock.addConsumer("5139", raffle.address);
    ...
    describe("fulfillRandomWords", function () {
        ...
        it("picks a winner, resets, and sends money", async () => {
            ...
            const winnerIndex = 2; //hardcoding for now
            ...
            await new Promise(async (resolve, reject) => {
                raffle.once("WinnerPicked", async () => {
                    ...
                    try {
                        ...
                        assert(raffleState.toString, "0");
                        //no players
                        await assert(raffle.getPlayer(0)).to.be.reverted;
                        assert(endingTimestamp > startingTimestamp);
                        assert()
                        const winner = raffle.getRecentWinner();
                        console.log(winner);
                        console.log(accounts[2].address);
                        console.log(accounts[3].address);
                        console.log(accounts[3].address);
                        const winnerEndingBalance = await accounts[winnerIndex].getBalance();
                        assert(winnerEndingBalance.toString(), winnerStartingBalance.add(entranceFee.mul(additionalEntrances)).add(entranceFee));
                        resolve();
                    }
                    catch (e) {
                        reject(e);
                ...
                console.log("1", (txReceipt.events[1].args).toString());
                console.log("2", (txReceipt.events[1].args.requestId).toString());
